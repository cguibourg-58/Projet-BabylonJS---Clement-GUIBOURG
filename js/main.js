...
let engine;
let scene;
var nombreDeMeurtre = 0;
var nombreDEnnemi = 10;
var textArrivee = "vous n'avez tue personne..."
var outputplane;
...
function createScene() {
  ...
  particleSystem.particleTexture = new BABYLON.Texture("image/flare.png");

  // Position where the particles are emitted from
  particleSystem.emitter = new BABYLON.Vector3(pointDArrivee.x, pointDArrivee.y, pointDArrivee.z);
  particleSystem.start();

  particleSystem.minLifeTime = 12;
  particleSystem.maxLifeTime = 12;

  particleSystem.direction1 = new BABYLON.Vector3(1, 20, 0);
  particleSystem.direction2 = new BABYLON.Vector3(-1, 20, 0);
...
function loadSounds(scene) {
  ...
  binaryTask = assetsManager.addBinaryFileTask("cantina", "sounds/cantina.mp3");
  binaryTask.onSuccess = function (task) {
    scene.assets.cantina = new BABYLON.Sound(
      "cantina",
      task.data,
      scene,
      null,
  ...
  binaryTask = assetsManager.addBinaryFileTask("victory", "sounds/marios-victory-theme.mp3");
  binaryTask.onSuccess = function (task) {
    scene.assets.victory = new BABYLON.Sound(
      "marios-victory-theme",
      task.data,
      scene,
      null,
...
function createGround(scene) {
  ...
  function onGroundCreated() {
    ...
    groundMaterial.diffuseTexture = new BABYLON.Texture("images/soil-texture-4.jpg");
...
function createArrivee(scene) {
  let arrivee = BABYLON.MeshBuilder.CreateSphere("heroTankCanon", {diameter: 20, segments: 32}, scene);
  arrivee.position.x = pointDArrivee.x;
  arrivee.position.y = pointDArrivee.y;
  arrivee.position.z = pointDArrivee.z;
  return arrivee;
}

function createTank(scene) {
  //let tank/*Mesh*/ = BABYLON.SceneLoader.ImportMesh("heroTank", "models/Dude/", "Dude.babylon", scene,  (newMeshes, particleSystems, skeletons) => {
  //});
  ...
  tank.move = () => {
    ...
    //console.log("z="+ tank.position.z+" x="+tank.position.x);

    var termine = false;

    if((tank.position.z >= pointDArrivee.z && tank.position.x >= pointDArrivee.x)&&!(tank.position.z >= (pointDArrivee.z+4) && tank.position.x >= (pointDArrivee.x+4))) {
      //termine = true;
      console.log('arrivee touched! and you killed '+nombreDeMeurtre+" soldiers!");
      textArrivee='arrivee touched! and you killed '+nombreDeMeurtre+" soldiers!";

      outputplane = BABYLON.Mesh.CreatePlane("outputplane", 100, scene, false);
      outputplane.billboardMode = BABYLON.AbstractMesh.BILLBOARDMODE_ALL;
      outputplane.material = new BABYLON.StandardMaterial("outputplane", scene);
      outputplane.position = new BABYLON.Vector3(pointDArrivee.x-15, 30, pointDArrivee.z-15);
      ...
      outputplaneTexture.drawText(textArrivee, null, 160, "bold 20px verdana", "white");
    
      outputplaneTexture.hasAlpha = true;

      if(nombreDeMeurtre>=nombreDEnnemi && !termine) {
        //createHeroDude(scene);
        //createScene();
        if(!termine){
          scene.assets.victory.play();
          ...
          outputplane.material.emissiveColor = new BABYLON.Color3(1, 1, 1);
          outputplane.material.backFaceCulling = false;
          outputplaneTexture.drawText("good job! all soldiers has been killed!", null, 120, "bold 20px verdana", "yellow");
  ...
  tank.fireCannonBalls = function () {
    ...
    }, 10000 * this.fireCannonBallsAfter);
      ...
      cannonball.actionManager.registerAction(
        new BABYLON.ExecuteCodeAction(
          ...
          () => {
            ...
            dude.Dude.gotKilled();
            nombreDeMeurtre++;
            console.log("nombreDeMeurtre : "+nombreDeMeurtre);
...
function createHeroDude(scene) {
  ...
  // tentative non réussie d'importer un modele 3d
  /*let meshTask = scene.assetsManager.addMeshTask(
    "Dude task",
    "him",
    "models/Jeep/",
    "t-90a(Elements_of_war).FBX"
  );*/
